...
<div class="page">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">${checks.map(c=>`<div style="background:${c.pass?'#eafaf1':'#fdecea'} !important;-webkit-print-color-adjust:exact;print-color-adjust:exact;border-radius:8px;padding:8px 10px;text-align:center;min-width:78px"><div style="font-size:20px;font-weight:900;color:${c.pass?'#1a6e1a':'#c0392b'}">${c.pass?'✓':'✗'}</div><div style="font-size:9px;font-weight:900;color:${c.pass?'#1a6e1a':'#c0392b'};margin:2px 0">${c.pass?'PASS':'FAIL'}</div><div style="font-size:8px;color:#444">${c.label}</div></div>`).join('')}</div>
